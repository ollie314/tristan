...
/**
  * The contracts service interface.
  * <p>
  * This describes everything that Lagom needs to know about how to serve and
  * consume the ContractsService.
  */
trait ContractsService extends Service {

  /**
    * Example: curl http://localhost:9000/api/contracts/{uuid}
    */
  def fetchContract(id: String): ServiceCall[NotUsed, String]

  /**
    * Example: curl -H "Content-Type: application/json" -X POST -d '{"message":
    * "Hi"}' http://localhost:9000/api/contracts/{uuid}
    */
  ...
  override final def descriptor = {
    ...
    named("contracts")
      .withCalls(
        pathCall("/api/contracts/:id", hello _),
